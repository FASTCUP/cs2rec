...
bool CModule::InitFromName(const std::string_view svModuleName, bool bExtension)
{
	...
	while (true)
	{
		size_t len = GetModuleFileNameA(handle, modulePath.data(), modulePath.length());
		if (len == 0)
			return false;
...
bool CModule::InitFromMemory(const CMemory pModuleMemory)
{
	...
	while (true)
	{
		size_t len = GetModuleFileNameA(reinterpret_cast<HMODULE>(mbi.BaseAddress), modulePath.data(), modulePath.length());
		if (len == 0)
			return false;
